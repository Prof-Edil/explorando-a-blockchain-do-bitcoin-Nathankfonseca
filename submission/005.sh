...
for i in $(seq 0 3);
do
pub[$i]=$(echo "$transacao" | jq -r ".vin[$i].txinwitness[1]")
done

multi=$(bitcoin-cli -rpcconnect=84.247.182.145:8332 -rpcuser=user_253 -rpcpassword=eRHxrWVSaWHw createmultisig 1 "[\"$pub[0]\",\"$pub[1]\",\"$pub3[2]\",\"$pub4[3]\"]")
